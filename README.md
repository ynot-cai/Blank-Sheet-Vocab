...
| `npm run test:kc-import` | 二期阶段 02（59 项）：提示词规则齐全、解析三级降级（非法 JSON）、逐块校验清洗 |
| `npm run test:kc-edit` | 二期阶段 03（66 项）：8 种块渲染、表格增删行列、搜索命中块内容、斩复活、批量 |
| `npm run test:kc-session` | 二期阶段 04（38 项）：会话结构、抽卡顺序、自评落库、保存恢复 |
| `npm run test:kc-exam` | 二期阶段 05（95 项）：三套提示词、三种解析、出题量/防重复/题库、mastery 重算、改分、语境词、三个接口 |
...
> `test:presets-ui` 和 `test:kc-ui` / `test:kc-import-ui` 单独跑：它们要起真实的预览/dev 服务 + 无头浏览器，
> `test:presets-ui` 验「点预设按钮 → 下载 → 建来源 → 合并页 → 确认入库 → IndexedDB 里真的有了」，
> `test:kc-ui` 验「`__kcselftest` 能跑通 + 攻击载荷插进真 DOM 后确实不执行」，
> `test:kc-import-ui` 验「**真打字 → 发送 → 卡片渲染 → 采纳 → 入库**」整条链路
> （它起一个**本地假 AI 服务**，所以**不需要密钥**、结果可复现）。
