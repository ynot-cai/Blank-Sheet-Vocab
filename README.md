...
**线上地址**：https://blank-sheet-vocab.vercel.app ｜ **仓库**：https://github.com/ynot-cai/Blank-Sheet-Vocab
（推送到 `main` → Vercel 自动构建上线；上线后先跑 `npm run test:live` 与 `npm run test:live-ui` 自查）
...
> 当前进度：**同步版阶段 01~07 + 二期（知识点精学）阶段 01~07 全部完成并已上线**，
> 另加了**改名**与**预设词库**。
...
点完会先弹**确认框**，在里面可以改这一档的**优先级**，确认后才进合并确认页。
...
> ⚠️ **预设的义项是没整理过的**：JSON 是从原始词表直接生成的，每个词只有 1 个义项、
> 里面塞着整串原文（例如 `access` → `"v. 获取 n. 接近，入口"`）。
> 导入后到「确认义项并入库」页点 **「AI 重新分析义项」**，让 AI 按下面的整理规范
> 把它们分类成义项、挑出代表词、把近义词逐个分开。

> **优先级只在这一刻有用**：它决定「同一个词在别的来源里已存在时，保留谁的义项」，
> 所以要在**导入前**的确认框里设好。事后再去设置页改来源优先级，
> 已经合并好的义项**不会**重新合并。
...
**唯一权威定义在 [`src/core/senseRules.ts`](./src/core/senseRules.ts)**——改规则只改那个文件，
AI 提示词会自动引用它（`src/services/ai.ts` 拼的是 `SENSE_RULES_FOR_AI`，不重抄一份）。
自检：`npm run test:sense-rules`（56 项）。

四步：

1. **分类成义项** —— 一个义项 = 一个独立核心含义
2. **挑代表** —— 每个义项挑一个最常用、最直白的说法当代表词
3. **近义词逐个分隔** —— 一个说法一项，**严禁打包**
4. **多词性判断同源** —— 同源 → 跨词性合并成同一义项；不同源 → 拆成不同义项
...
「跑步 / 短途跑步」标的是名词性，但和动词性的「跑」**是同一个含义** → 同源 → 并进①，
且①的代表词**不加词性前缀**（它是跨词性的）。

反例 `bear n. 熊 v. 忍受`：「熊」和「忍受」不相干，只是拼写相同 → 不同源 → **必须拆成两个义项**。

### 两条硬约束（代码强制执行，不只是提示词里的请求）

| 约束 | 违反的后果 | 执行点 |
|---|---|---|
| **一个近义词里只能有一个中文说法** | `aliases:["跑步，奔跑"]` 归一化后是 `"跑步奔跑"`，用户答「跑步」或「奔跑」**都判错**（实测过）。混英文同样不行 | `normalizeAliases()`，在 `createSense()` 与 AI 解析兜底各调一次 |
| **「点单词显示中文」只显示代表词** | 一个义项可能有七八个近义词，全铺白纸上会糊成一片；所以代表词必须能独立看懂 | `formatSensesBrief()` 只读 `s.text` |
...
| `npm run test:sense-rules` | 资料整理规范：AI 提示词真的含规则、近义词拆包、**打包会判错**的回归防线、只显示代表词 |
| `npm run test:presets` | 预设词库：拿原始词表**独立复算**、各档两两不相交、清单与产物一致 |
| `npm run test:kc` | 二期阶段 01（106 项）：掌握度公式、**安全渲染（XSS）**、**空间隔离**、软删除墓碑传播、断网可用、时钟倒退不漏推、查询纯函数、四张表复合主键、一期回归 |
| `npm run test:kc-import` | 二期阶段 02（95 项）：提示词规则齐全、解析三级降级（非法 JSON）、逐块校验清洗、**对话历史（含截断）** |
| `npm run test:kc-edit` | 二期阶段 03（69 项）：8 种块渲染、表格增删行列、搜索命中块内容、斩复活、批量 |
| `npm run test:kc-session` | 二期阶段 04（38 项）：会话结构、抽卡顺序、自评落库、保存恢复 |
| `npm run test:kc-exam` | 二期阶段 05（123 项）：三套提示词、三种解析、出题量/防重复/题库、mastery 重算、改分、语境词、三个接口、**批量出题的槽位与 Enter 规则表** |
| `npm run test:kc-review` | 二期阶段 06（49 项）：抽卡规则、桥接词源、四阶段流转、收尾属性 |
| `npm run test:kc-settings` | 二期阶段 07（58 项）：表达式引擎、改参数后掌握度变化、题库分类与导入导出 |

**打线上用的两套**（会走网络，所以不在 `npm test` 里）：

| 命令 | 验什么 |
|---|---|
| `npm run test:live` | 线上 `https://blank-sheet-vocab.vercel.app` 的 9 条 `/api/*` 路由 + 一期同步协议（401 / 隔离 / 500 上限 / 白名单），跑完自动清理测试数据 |
| `npm run test:live-ui` | **线上那一份构建**在真 Chrome 里能不能挂载、二期六个入口在不在、**设置页开不开得起来**、一期首页没崩（只读，不写数据） |

`npm test` 会把上面全部跑一遍并做类型检查。**改完代码先跑它。**

> `test:presets-ui` 和 `test:kc-ui` / `test:kc-import-ui` / `test:kc-edit-ui` / `test:kc-session-ui` /
> `test:kc-review-ui` / `test:kc-e2e` / `test:kc-exam-ui` 单独跑：它们要起真实的预览/dev 服务 + 无头浏览器，
> `test:presets-ui` 验「点预设按钮 → 下载 → 建来源 → 合并页 → 确认入库 → IndexedDB 里真的有了」，
> `test:kc-ui` 验「`__kcselftest` 能跑通 + 攻击载荷插进真 DOM 后确实不执行 + **设置页在某个分区坏掉时不整页白**」，
> `test:kc-import-ui` 验「**真打字 → 发送 → 卡片渲染 → 采纳 → 入库**」整条链路，
> `test:kc-exam-ui` 验「**答题过程中零出题请求**（一口气出完）+ Enter 键全程可用」。
> （这几套都起一个**本地假 AI 服务**，所以**不需要密钥**、结果可复现。）
> 这类**构建产物/真引擎层面**的问题本地纯 Node 测不出来（和 §「两条本地测不出来的教训」同一类），
> 所以放在需要浏览器的那一档；本机没有 Chrome/Edge 时它们会自动跳过而不是失败。
>
> ⚠️ 本地全绿 **不等于** 线上能用：本地的浏览器套件跑的都是 dev 源码，`test:live` 只打接口。
> 「线上构建能不能打开」只有 `test:live-ui` 能回答。
...
| 入口 | 做什么 |
|---|---|
| 录入 | 粘贴/上传 `.txt/.csv/.md`，AI 智能解析（分批 + 断点续传）或规则解析（离线），再进合并确认页。AI 会主动拆义项（「量纲、维度」拆成两个）、为每个义项找近义词放进 aliases。**也可以直接点预设词库一键导入**（见下）。合并页还有 **「AI 重新分析义项」**：把已解析的词交回 AI 按整理规范重排义项（预设导入后整理义项就用它） |
...
| 单词列表 | 统计条 / 搜索 / 筛选 / 批量操作 / 斩词复活 / 重算优先度 / 清空全部单词 / 导出导入（手机上自动换成卡片流）。**批量编辑不设数量上限**：表头全选或「全选筛选结果」可一次选中跨页的全部结果 |
| 设置 | A 星号参数 / B AI 接口 / C 画面与纸张 / D 记忆与练习 / E 复习优先度 / **F 云同步** / **G 数据（备份恢复清空）** |

首页底部有「关于数据」入口（`#/about`）：讲清数据存在哪、服务器存什么、怎么删。

### 二期（知识点精学）—— 阶段 02 已完成：录入可用

一期是**泛背**（单词量大、浅层），二期是**精学**（知识点少、深层）。
数据单位从结构固定的 `Word` 换成**结构自由的 `KnowledgeCard`**（由 Block 块组合而成：
小标题 / 正文 / 例句 / 列表 / 表格 / 代码 / 引用 / 提示）。

顶栏点「知识点」进入二期（`#/kc`）。**阶段 02 起「录入」已经能用了**：

| 阶段 | 内容 | 状态 |
|---|---|---|
| 01 | 数据层（4 张表 / DAO / 同步）+ 安全块渲染 | ✅ |
| 02 | **聊天式录入**：说一句「我这块不行」→ AI 拆成知识点卡片 → 预览/采纳/编辑 → 确认入库 | ✅ |
| 03 | 块编辑器 + 卡片列表（增删改、斩/复活） | ✅ |
| 04 | 学习流程（选数量 → 看卡片 → 自评三级 → 斩） | ✅ |
| 05 | 出题与评分（每日语境词 + AI 出题 + rubric 评分 + 题库参考） | ✅ |
| 06 | 复习流程（看卡片 → 背 5 词桥接一期 → 做题 → 收尾） | ✅ |
| 07 | 设置页（mastery 公式 + 优先度表达式）、题库管理页 | ✅ |

**二期已全部完成。** 二期首页（顶栏「知识点」）**六个**入口：录入 / 卡片列表 / 学习 / 复习 / 题库 / 设置。

> **做题没有独立入口**（用户明确要求撤掉）：它是「学习」和「复习」流程里的一步。
> 学习到一半退出会保存进度；**做题到一半退出，再点「学习」会直接回到你刚才那道题**。

> 一二期唯一的桥接点是**复习流程中间那一步「背 5 个单词」**：跳去一期的白纸背单词界面（词来自一期词库，
> 优先新词，没有新词就复习旧词，词库为空则自动跳过），背完回到二期继续做题。
> 除此之外两套数据完全独立（独立表、独立 DAO、独立路由），只共用同一套同步通道。

**录入怎么用**：顶栏「知识点」→「录入」→ 用大白话输入（如「charge 的短语用法」）→
AI 会拆成 2~5 个独立知识点，每张卡**只留记忆痛点**（2~4 个块、每块一句话，优先「有 the 时…／无 the 时…」
这种二分对照；表格**不带表头**）→ 逐张「预览」看清楚 → 点「采纳」→「确认入库（N 张）」。
**同一个聊天里可以接着追问**（如「第二个再细一点」）：AI 会带上之前的对话作为上下文。

> AI 密钥只存在你这台设备的浏览器里（设置 → B 区），服务器全程不接触（方案 B）。
> 提示词集中在 `src/services/kcPrompts.ts`：如果 AI 拆得不好，**只改那一个文件**即可。
> 上下文长度上限在 `src/core/config.ts`（`maxChatHistoryMessages` / `maxChatHistoryChars`），
> 超出时聊天区会显示「已省略较早的 N 轮对话」。

**做题怎么用**：进入做题时会**先一口气把所有题出完**（界面上有进度条），
之后答题过程中不再等 AI 出题；Enter 键**全程有效**：填空/造句提交、选择题 ↑↓ 换项后 Enter 确认、
评分完 Enter 直接下一题、出错 Enter 重试。造句题里 Shift+Enter 才是换行。
...
二期（知识点）另挂一个入口，同样**不自动执行**：

```js
await __kcselftest.run()      // 全部：mastery 公式 / 安全渲染 / 二期数据层
__kcselftest.mastery()        // 掌握度公式（纯函数，同步返回）
__kcselftest.xss(true)        // 安全渲染，true = 把测试内容留在页面上肉眼确认
await __kcselftest.data()     // 卡片 DAO：增删改查 / 斩 / 复活 / 分页
```

> `xss()` 会真的往页面里插一段含 `<script>` / `<img onerror=...>` 的内容，
> 用来确认它们**只显示成纯文本**（不弹窗、不发请求）；不加参数则跑完自动移除。
...
├─ _lib/                   db（Turso 单例 + 一期建表）/ spaceAuth / cors / http / limits / validate
│                          + kcSchema（二期建表）/ kcInventory（二期 SQL）/ kcValidate（二期校验）
...
├─ kc-list.ts              GET  /api/kc-list?since=<ts>（二期：知识点卡片增量拉取）
├─ kc-push.ts              POST /api/kc-push（二期：单批 ≤ 500 张）
