...
> 真机视口的几何验收由 `npm run test:m2-ui`（CDP 精确设视口）覆盖，20 项全过。
...
| `npm run test:m2-ui`（20 项） | **CDP 精确视口 390×844**：词数/重叠/越界、圆按钮数量/直径/一行/居中、避让带=底部横带、按钮全部落在避让带内、**有词落在按钮左侧**、桌面仍是旧算法+右下角方块 |
...
| `test:m2-ui` | 20 | **CDP 精确视口 390×844** 的真机验收：词数 / 重叠 / 越界 / 圆按钮几何 / 避让带 / 词在按钮左侧 / 桌面未被误改 |
