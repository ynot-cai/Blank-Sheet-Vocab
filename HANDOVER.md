...
### 0.18 阶段 S3：桌面端「像手机一样只排成两列」的回归修复 + 手动列数覆盖（★ 当前最新改动）

用户原话：「电脑上出现的单词像手机一样只排成两列」。

#### 先说结论：S3 提示词里的根因推断**只对了一半**，实测修正如下

| 推断 | 实测结论 |
|---|---|
| 「`computeGrid` 里 `if (capacity >= target) break` 导致宽屏 2 列」 | **只对手机成立**。`computeGrid` 是 M2 为手机写的，桌面/平板**从来没走过它**（`PaperStage` 里 `if (kind === 'phone')` 分支） |
| 「桌面因此退化成 2 列」 | 症状真实存在，但**病因是另一处**：桌面/平板仍走旧的 `jitteredGrid`，它用 `minGapW = (最宽词 + 字号×2.4)/屏宽` 去夹列数 —— 一个长词就能把列数压死（M1 查出来的那个病，M2 只给手机治了） |

**桌面 2 列的机器证据**（真实背诵页：字号 24、最宽词 photosynthesis = 177.9px、队列 20 个词）：

```
最小中心距 = 177.9 + 24×2.4 = 235.5px
minGapW   = 235.5 / 屏宽
cols      = min( round(√(count×宽高比)) , floor(0.88 / minGapW) )      ← 后一项永远更小
```

| 窗口 | minGapW | 夹出来的列数 | 要的列数 | 旧算法实际列数 | 旧容量（带避让） |
|---|---|---|---|---|---|
| 1920×1080 | 0.1227 | 7 | 6 | 6 | 20 |
| 1280×800 | 0.1840 | **4** | 6 | **4** | 20 |
| 1024×768 | 0.2300 | **3** | 5 | **3** | 17 |
| 900×800 | 0.2617 | **3** | 5 | **3** | 16 |
| 820×800 | 0.2872 | **3** | 5 | **3** | 16 |
| **800×800 / 780×800** | 0.2944 / 0.3019 | **2** | 4 | **2** | **11（16 个词只放得下 11 个）** |
| 390×844 | 0.6038 | 1 | 3 | 1 | 6 |

（复算脚本口径写在 `core/layout.ts` 顶部注释里；数字与真实页面实测一致：1280→4 列、900→3 列、820→3 列、780→2 列且只放得下 10 个词。）

#### 修法：全端统一成「自然列数」网格（`core/layout.ts` 的 `computeGrid` 重写）

1. **列数由宽度自然推导**（不再「够放 target 个就停」，也不再被最宽词夹死）：
   `maxCols = ⌊可用宽 / (平均词宽×1.15 + 最小空隙)⌋`，自动值取
   `clamp( round(√(目标词数 × 可用区宽高比)), minCols, maxCols )`；
2. **列数硬下限**（防退化，参数在 `core/config.ts` 的 `DEVICE`）：手机 3 / 平板 4 / 桌面 4；
3. **行数按目标词数反推**：`rows = clamp(⌈target / cols⌉, 1, ⌊可用高/格高⌋)` —— 容量仍由 `targetCount` 决定；
4. **格子撑满可用区域**（格宽 = 可用宽/列数、格高 = 可用高/行数），N 个词**随机落格**
   → 均匀散布全屏，不会全挤在顶部两行；
5. **真实矩形碰撞检测**（M2 的 `layoutWords`）继续兜底：不重叠 / 不越界 / 不压按钮；
6. 平板/桌面的可用区改成「整张纸扣边距」（右下角按钮是**方块**，按真实矩形判交避开），
   不再把整条底部横带封掉 —— 按钮左边那一大片空白也能放词了；
7. 旧 `jitteredGrid` / `spacingBudget` **保留但退役**（`paperLayout` 与对照测试仍在用，
   文件顶部注释写清了它为什么不能再给真实白纸布点）。

#### 手动列数覆盖（永久兜底，用户明确要求）

| 项 | 做法 |
|---|---|
| 设置 | `settings.layoutColsOverride`：`'auto'`（默认）/ 3 / 4 / 5 / 6 / 8 / 10，脏值一律回落 `'auto'`（`coerceColsOverride`） |
| 设置页 | 「C2. 布局参数」顶部新增「列数」下拉 |
| 背诵页 | **32×32 的 ⊞ 按钮** + 轻量选择条（不是全屏弹窗）：手机在底部按钮带左侧、平板/桌面在右下角按钮列最上面（两处都是布点避让区，压不到单词）；选中**立刻重排**（`PaperStage.relayout()` 就地移动已上纸的词）并**写设置记住** |
| 兜底语义 | 给了具体列数就忽略自动推导与 `minCols`；但宽度放不下时会被夹住，并在 `__layoutInfo.colsClamped` 如实报告（否则必然叠字） |

#### 修复前 vs 修复后（**真实背诵页**，16 词已上纸，CDP 精确视口）

| 窗口 | 修复前列数 | 修复后列数 | 修复后网格 | 词数 | 修复后纵向铺开 |
|---|---|---|---|---|---|
| 1920×1080 | 7（网格 6 列） | **13** | 9 列 × 5 行 | 16 | 87% |
| 1536×864 | 6 | **13** | 9 × 5 | 16 | 87% |
| 1366×768 | 5 | **13** | 9 × 5 | 16 | 87% |
| 1280×800 | 4 | **10** | 8 × 5 | 16 | 87% |
| 1024×768 | 6 | **10** | 6 × 4 | 16 | 85% |
| 900×800 | 3 | **9** | 5 × 5 | 16 | 90% |
| 820×800 | 3 | **8** | 5 × 5 | 16 | 90% |
| **780×800** | **2（只放得下 10 个词）** | **8** | 5 × 5 | **16** | 90% |
| 820×1180（平板） | 3 | **9** | 4 × 6 | 16 | 88% |
| 390×844（手机） | 6（网格 2 列） | 6 | 3 × 6 | 16 | 74% |

> 手机的数字是**故意保留**的：词数 15~18、不重叠、不越界、不压按钮带全部不变；
> 变的只是网格从「2 列 × 12 行」变成「3 列 × 6 行」（M2 的 2 列本身就是
> 「够放 16 个就停」的产物）。纵向范围 27~651px 前后一致。

#### 四个场景探针（`probeLayout.mjs`，S3 新增 `--min-columns` / `--cols-override` / `--min-y-coverage`）

| 场景 | viewport | 断言 | 实测 |
|---|---|---|---|
| 手机 | 390×844 | columns ≥3、词数 15~18 | columns=6、词数 16、minGap=16.48、y 覆盖 74% |
| 平板 | 820×1180 | columns ≥4 | columns=9（网格 4 列）、y 覆盖 84% |
| 桌面 | 1280×800 | **columns ≥5** | columns=11（网格 8 列）、y 覆盖 86% |
| 大屏 | 1920×1080 | **columns ≥6** | columns=11（网格 9 列）、y 覆盖 84% |
| 手动覆盖 | 1280×800 + 6 列 | 网格 = 6 且 DOM 落点都在 6 列内 | 6 列 × 7 行，`grid.cols↔DOM = 6/6 列有词，越列 0` |

每个场景都是 `overlapPairs=0 / outOfBounds=0 / buttonOverlaps=0`；
`spread.yCoverage` 是 S3 新加的指标（词在纵向铺开的比例，默认要求 ≥ 0.5）。

> ⚠️ 顺手修了两个**测量口径**的坑（否则数字会假）：
> 1. 调试页的 `tier=desktop` 以前**只改档位名、不换那一档的参数**，
>    于是「桌面场景」量到的其实是手机档参数（边距 8 / 目标 16 / 间距 8）；
> 2. `probeLayout.mjs` 的 `viewport` 断言比的是 `window.innerWidth`，
>    而 Windows 无头窗口宽受物理屏幕限制（本机 1707px），量 1920 宽的布局会凭空多出
>    「越界」假失败 → 改成断言**布点实际用的视口**（注入的模拟真机尺寸）。

#### 新增/调整的测试

| 命令 | 覆盖 |
|---|---|
| `npm run test:s3-ui`（**新增**，30 项） | 四场景探针 + 手动覆盖探针 + **真实背诵页**：桌面列数 ≥5 / 大屏 ≥6 / 平板 ≥4、均匀散布 ≥50% 纸高、**⊞ 32×32 能点、弹选择条、选 6 列立刻重排、写进设置、刷新后位置仍是那一套**、桌面按钮仍右下角、手机词数 15~18 与 ⊞ 落在避让带内 |
| `npm run test:m2-ui`（41 项） | 手机部分一字未改；桌面那条改成 S3 口径（自然列数 + 列数 ≥5） |
| `npm run test:mobile`（59 项） | 未改（`computeGrid` 的容量/格高/长词对照全绿） |
| `npm run test:paper`（79 项） | [2e] 改成 S3 契约：调 `computeGrid`/`layoutWords`、不再用 `jitteredGrid` 布点、避让区按真实矩形判交 |

#### 教训（值得单独记一条）

**「修好一个端」不等于「修好这个病」**。M2 给手机换了算法并留了详细诊断注释，
但桌面的同一处病根（最宽词夹列数）没有一起处理 —— 于是半年后以
「电脑上像手机一样两列」的形式重新出现，而且**调试页量不出来**
（调试页的演示词最宽只有 114.7px，夹不出 2 列；真实词库里有 photosynthesis）。
两条通用做法因此固化下来：
1. 诊断脚本要用**真实数据的最坏情况**（长词表）跑，不能只用演示数据；
2. 布局这类「同一个函数服务多个端」的改动，验收要**逐端列数字**（本节的四场景表）。
